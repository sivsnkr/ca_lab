		...
		if(fu.clockCycle == 0) {
			...
			if(unitType == 'add') {
				if(inst.includes('ADD')) {
					...
					let multiply = exec(`iverilog -o ${execName} -DA=${TempRegisters[thisIntruction.fj]} -DB=${TempRegisters[thisIntruction.fk]} -Dcin=0 ../Adder/Adder_tb.v`, (err, stdout, stderr) => {
						if(err) throw err;
					...
					let multiply = exec(`iverilog -o ${execName} -DA=${TempRegisters[thisIntruction.fj]} -DB=${TempRegisters[thisIntruction.fk]} -Dcin=1 ../Adder/Adder_tb.v`, (err, stdout, stderr) => {
						if(err) throw err;
						let runScript = exec(`./${execName} > ${fileName}`, (err, stdout, stderr) => {
							if(err) throw err;
						})
				...
				TempRegisters[thisIntruction.fi] = TempRegisters[thisIntruction.fj] * TempRegisters[thisIntruction.fk];
				let multiply = exec(`iverilog -o ${execName} -DA=${TempRegisters[thisIntruction.fj]} -DB=${TempRegisters[thisIntruction.fk]} ../Wallace_multiplier/Wallace_tb.v`, (err, stdout, stderr) => {
					if(err) throw err;
					let runScript = exec(`./${execName} > ${fileName}`, (err, stdout, stderr) => {
						if(err) throw err;
					})
				...
				if(thisIntruction.inst.includes('FADD')) {
					...
					TempRegisters[thisIntruction.fi] = TempRegisters[thisIntruction.fj] + TempRegisters[thisIntruction.fk];
					let fadd = exec(`iverilog -o ${execName} -DA=${finalA} -DB=${finalB} -DaddOrSub=0 ../Floating_p_addr/FPA_tb.v`, (err, stdout, stderr) => {
						if(err) throw err;
						let runScript = exec(`./${execName} > ${fileName}`, (err, stdout, stderr) => {
							if(err) throw err;
						})
				...
				if(thisIntruction.inst.includes('FMUL')) {
					...
					let finalB = TempRegisters[thisIntruction.fk] > 0 ? "0".concat(fke.concat(fkm)) : "1".concat(fke.substr(1).concat(fkm));
					TempRegisters[thisIntruction.fi] = TempRegisters[thisIntruction.fj] * TempRegisters[thisIntruction.fk];
					let fadd = exec(`iverilog -o ${execName} -DA=${finalA} -DB=${finalB} ../multiplier/tb.v`, (err, stdout, stderr) => {
						if(err) throw err;
						let runScript = exec(`./${execName} > ${fileName}`, (err, stdout, stderr) => {
							if(err) throw err;
						})
