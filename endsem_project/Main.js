...
var { Instructions, Memory, TempRegisters } = require("./InstructionUnit");
...
class MarkParser {
  ...
  parseFunctionalUnit(token) {
    ...
    for (let i = 0; i < noOfUnits; i++) {
      this.Mark.functionalUnits.push(new FunctionalUnit(unitType, clockCycle));
    }
  }
  ...
  parseInstruction(token) {
    ...
    this.Mark.instructions.push(tokenizedInstruction);
  }
}
...
class Mark {
  ...
  checkComplete() {
    ...
    if (remainingInstructions) {
      for (let i = 0; i < this.functionalUnits.length; i++) {
        if (this.functionalUnits[i].busy) {
          // Check if any FU is working after instructions getting over
  ...
  fuCanIssue(fu, instr) {
    ...
    return (
      !fu.busy &&
      fu.unitType == instr.unitType &&
      !(instr.fi in this.registerStatus)
    ); // Check for WAW and FU is free or not
  }
  ...
  readToFu(fu) {
    fu.readOperands();
    this.instructions[fu.instPC].readOp = this.clock; //  Set operands read time in Mark
  }
  ...
  execToFu(fu) {
    ...
    if (fu.clockCycle == 0) {
      this.instructions[fu.instPC].execInst = this.clock; // Store execution CC in Mark for this instruction
      let thisIntruction = this.instructions[fu.instPC];
      let unitType = thisIntruction.unitType;
      let inst = thisIntruction.inst;
      let execName =
        thisIntruction.inst.split(" ")[0] + thisIntruction.issue + ".vvp";
      let fileName =
        thisIntruction.inst.split(" ")[0] + thisIntruction.issue + ".txt";
      if (unitType == "add") {
        if (inst.includes("ADD")) {
          TempRegisters[thisIntruction.fi] =
            TempRegisters[thisIntruction.fj] + TempRegisters[thisIntruction.fk];
          //   let multiply = exec(
          //     `iverilog -o ${execName} -DA=${
          //       TempRegisters[thisIntruction.fj]
          //     } -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dcin=0 ../Adder/Adder_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
        ...
        //   } -DB=${
        //     TempRegisters[thisIntruction.fk]
        //   } ../Wallace_multiplier/Wallace_tb.v`,
        //   (err, stdout, stderr) => {
        //     if (err) throw err;
        //     let runScript = exec(
        //       `./${execName} > ${fileName}`,
        ...
        if (inst.includes("CMP")) {
          ...
          //   let cmp = exec(
          //     `iverilog -o ${execName} -DA=0 -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dselect=1 ../logical/logical_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
          ...
          //     } -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dselect=2 ../logical/logical_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
          ...
          //     } -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dselect=3 ../logical/logical_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
          ...
          //     } -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dselect=4 ../logical/logical_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
          ...
          //     } -DB=${
          //       TempRegisters[thisIntruction.fk]
          //     } -Dselect=5 ../logical/logical_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
        ...
        if (thisIntruction.inst.includes("FADD")) {
          let fje = parseInt(
            TempRegisters[thisIntruction.fj].toString().split(".")[0]
          ).toString(2);
          let fjm = parseInt(
            TempRegisters[thisIntruction.fj].toString().split(".")[1]
          ).toString(2);
          let fke = parseInt(
            TempRegisters[thisIntruction.fk].toString().split(".")[0]
          ).toString(2);
          let fkm = parseInt(
            TempRegisters[thisIntruction.fk].toString().split(".")[1]
          ).toString(2);
          fje = ("00000" + fje).slice(-5);
          fjm = ("0000000000" + fjm).slice(-10);
          let finalA =
            TempRegisters[thisIntruction.fj] > 0
              ? "0".concat(fje.concat(fjm))
              : "1".concat(fje.substr(1).concat(fjm));
          fke = ("00000" + fke).slice(-5);
          fkm = ("0000000000" + fkm).slice(-10);
          let finalB =
            TempRegisters[thisIntruction.fk] > 0
              ? "0".concat(fke.concat(fkm))
              : "1".concat(fke.substr(1).concat(fkm));
          TempRegisters[thisIntruction.fi] =
            TempRegisters[thisIntruction.fj] + TempRegisters[thisIntruction.fk];
          //   let fadd = exec(
          //     `iverilog -o ${execName} -DA=${finalA} -DB=${finalB} -DaddOrSub=0 ../Floating_p_addr/FPA_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
          ...
          TempRegisters[thisIntruction.fi] =
            TempRegisters[thisIntruction.fj] - TempRegisters[thisIntruction.fk];
          //   let fadd = exec(
          //     `iverilog -o ${execName} -DA=${finalA} -DB=${finalB} -DaddOrSub=1 ../Lab3/FPA_tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
        ...
        if (thisIntruction.inst.includes("FMUL")) {
          ...
          TempRegisters[thisIntruction.fi] =
            TempRegisters[thisIntruction.fj] * TempRegisters[thisIntruction.fk];
          //   let fadd = exec(
          //     `iverilog -o ${execName} -DA=${finalA} -DB=${finalB} ../multiplier/tb.v`,
          //     (err, stdout, stderr) => {
          //       if (err) throw err;
          //       let runScript = exec(
          //         `./${execName} > ${fileName}`,
  ...
  fuCanWrite(fu) {
    ...
    for (let i = 0; i < this.functionalUnits.length; i++) {
      // Check for WAR
      canWriteBack =
        (this.functionalUnits[i].fj != fu.fi || !this.functionalUnits[i].rj) &&
        (this.functionalUnits[i].fk != fu.fi || !this.functionalUnits[i].rk);
      if (!canWriteBack) {
        break;
      }
    }
  ...
  tickClock() {
    ...
    let nextInstruction = this.hasRemainingInstructions()
      ...
      : null; // Get new instruction if there is any else don't

    this.functionalUnits.forEach((fu) => {
      if (this.fuCanIssue(fu, nextInstruction)) {
        // Check for issue
    ...
    this.functionalUnits.forEach((fu) => {
      if (!fu.lock && this.fuCanWrite(fu)) {
        // Check if some instruction can be read
        this.writeToFu(fu);
      }
    });
...
function main() {
  sb = new MarkParser().asmFileParser(process.argv[2]);
  // sb contains only the FUs and Instrs after parsing
  while (!sb.checkComplete()) {
    // Check if done
